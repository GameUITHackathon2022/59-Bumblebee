namespace Dialogue {
    ...
    public abstract class TextAnimation : MonoBehaviour
    {
        ...
        /// </summary>
        private const float FRAME_RATE = 15f;
        private static readonly float timeBetweenAnimates = 1f / FRAME_RATE;
        ...
        protected int firstCharToAnimate
        {
            get
            {
                return this._firstCharToAnimate;
            }
        }
        protected int lastCharToAnimate
        {
            get
            {
                return this._lastCharToAnimate;
            }
        }
        ...
        {
            this._firstCharToAnimate = firstChar;
            this._lastCharToAnimate = lastChar;
        }
        ...
        {
            this._textInfo = this.textComponent.textInfo;
            this._cachedMeshInfo = this._textInfo.CopyMeshInfoVertexData();
        }

        protected virtual void Awake()
        {
            this.enabled = this._playOnAwake;
        }

        protected virtual void Start()
        {
            this.textComponent.ForceMeshUpdate();
            this._lastAnimateTime = float.MinValue;
        }
        ...
        {
            if (this.timeForTimeScale > this._lastAnimateTime + timeBetweenAnimates)
            {
        ...
        {
            this._lastAnimateTime = this.timeForTimeScale;

            int characterCount = this._textInfo.characterCount;
            ...
            for (int i = 0; i < characterCount; i++)
            {
                ...
                if (i < this._firstCharToAnimate || i > this._lastCharToAnimate)
                {
                    continue;
                }
        ...
        {
            for (int i = 0; i < this._textInfo.meshInfo.Length; i++)
            {
                this._textInfo.meshInfo[i].mesh.vertices = this._textInfo.meshInfo[i].vertices;
                this.textComponent.UpdateGeometry(this._textInfo.meshInfo[i].mesh, i);
            }
        }
