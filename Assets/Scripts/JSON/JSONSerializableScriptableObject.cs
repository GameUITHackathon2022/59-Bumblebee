...
public class JSONSerializableScriptableObject<T> : ScriptableObject where T : JSONSerializableScriptableObject<T>
{
    public void LoadJsonFromFile(string filename)
    {
        if (!File.Exists(filename))
        {
            Debug.LogError($"File name {filename} does not exist!");
            return;
        }
